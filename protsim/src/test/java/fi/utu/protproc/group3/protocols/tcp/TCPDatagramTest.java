...
class TCPDatagramTest {
    ...
    void createDatagram() {
        byte[] payload = { 0x01 };
        var datagram = TCPDatagram.create(sourcePort, destPort, 0, 0, flags, (short) 1, payload);

        assertNotNull(datagram);

        var bytes = datagram.serialize(sourceIP, destinationIP);
    ...
    void reassembleDatagram() {
        ...
        var original = TCPDatagram.create(sourcePort, destPort, 5, 6, flags, (short) 1, payload);

        assertNotNull(original);

        var bytes = original.serialize(sourceIP, destinationIP);
    ...
    void parseTCPDatagram() {
        var pdu = StringUtils.parseHexStream("");
        ...
        var rebuilt = EthernetFrame.create(
                ...
                IPv6Packet.create(packet.getVersion(), packet.getTrafficClass(), packet.getFlowLabel(),
                        ...
                        TCPDatagram.create(datagram.getSourcePort(), datagram.getDestinationPort(), datagram.getSeqN(),
                                datagram.getAckN(), datagram.getFlags(), datagram.getWindow(), datagram.getPayload()
                        ).serialize(packet.getSourceIP(), packet.getDestinationIP())
                ).serialize()
        ).serialize();
    ...
    void checksumIsCorrect() {
        var pdu = StringUtils.parseHexStream("226ba8349d40265a71af73b986dd600000000014067f200300030000000051794ab90b8cf10d2003000300000000e6c4a65b959c0758cbcd005010a00f30000000005002ffffc0df0000");
        var frame = EthernetFrame.parse(pdu);
        var packet = IPv6Packet.parse(frame.getPayload());
        var datagram = TCPDatagram.parse(packet.getPayload());

        datagram.serialize(packet.getSourceIP(), packet.getDestinationIP());

        assertEquals((short) 0xc107, datagram.getChecksum());
    }
}
