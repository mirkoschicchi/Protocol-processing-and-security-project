...
public class RouterNodeImpl extends NetworkNodeImpl implements RouterNode {
    ...
    public RouterNodeImpl(Iterable<EthernetInterface> interfaces, Thread backgroundThread, NetworkAddress networkAddress) {
        super(interfaces, backgroundThread, networkAddress);
    }
    ...
    @Override
    public void route() {
        // Get all the router interfaces
        for(EthernetInterface ethernetInterface: getInterfaces()) {
            // Get all the packets in the queue of a specific interface
            Queue<byte[]> receiverQueue = ethernetInterface.getReceiverQueue();
            ...
            for (byte[] pdu: receiverQueue) {
                ...
                byte[] nextHopMac = ethernetInterface.resolveInetAddress(nextHop.getAddress());
                ...
                // Route the frame
                ethernetInterface.transmit(serializedFrame);
            }
        }
    }
