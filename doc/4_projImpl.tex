...
To have an easier view of the exchanged messages and check the correctness of the protocol algorithms, we decided to export a PCAP file after every run.\newline
There is the possibility to avoid the PCAP export just using a flag from the command line as you can see in the usage chapter \ref{simulatorDoc}.\newline
...
Our simulated IEEE 802.3 Ethernet allows an infinite number of octets as MTU.
To reduce the workload, we decided to not implement the Preamble, start frame delimiter (SFD), frame check sequence (FCS) and the interframe gap (IFG).
Given that the project is in a simulation environment, all these components were not necessary to have correct BGP functionalities.\newline
Then, we created our own data structures to simplify further the data handling.
These structures refer to:
\begin{itemize}
    \item The way we resolve the IP (replacing the ARP protocol)
    \item How we enumerate the devices (Neighbor Discovery Protocol replacement)
\end{itemize}
